...
#import "XHDisplayTextViewController.h"   //文字
#import "XHDisplayMediaViewController.h"    //视频
#import "XHDisplayLocationViewController.h"  //位置
#import "XHAudioPlayerHelper.h"     //音频

#import "LZStatusView.h"
#import "CDEmotionUtils.h"     //表情
...
- (void)viewDidLoad {
    ...
    userItem.nickname = [_dic objectForKey:@"nickname"];
    userItem.uuid = [_dic objectForKey:@"uuid"];
    userItem.imageUrl = [_dic objectForKey:@"avatar"];
    userItem.is_favorite = [_dic objectForKey:@"is_favorite"];
    FMDBManager * manager = [FMDBManager sharedManager];
    [manager insertUser:userItem];
...
//    id<CDUserModel> selfUser = [[CDChatManager manager].userDelegate getUserById:[CDChatManager manager].selfId];
    UserItem * item = [[FMDBManager sharedManager] getAllNicknameAndImageUrl:[CDChatManager manager].selfId];
    self.messageSender = item.nickname;
...
- (void)refreshConv {
    NSLog(@"self.conv.title = %@",self.conv.title);
...
- (void)sendMsg:(AVIMTypedMessage *)msg {
    ...
    NSLog(@"self.conv.createAt = %@",self.conv.createAt);
    NSLog(@"self.conv.members = %@",self.conv.members);
...
//    id <CDUserModel> fromUser = [[CDChatManager manager].userDelegate getUserById:msg.clientId];
    NSLog(@"clientId = %@",msg.clientId);
    UserItem * userItem = [[UserItem alloc]init];
    userItem = [[FMDBManager sharedManager] getAllNicknameAndImageUrl:msg.clientId];
    XHMessage *xhMessage;
    NSLog(@"nickname = %@",userItem.nickname);
    ...
    if (msg.mediaType == kAVIMMessageMediaTypeText) {
        ...
        xhMessage = [[XHMessage alloc] initWithText:[CDEmotionUtils emojiStringFromString:textMsg.text] sender:userItem.nickname timestamp:time];
    }
        ...
        xhMessage = [[XHMessage alloc] initWithVoicePath:audioMsg.file.localPath voiceUrl:nil voiceDuration:duration sender:userItem.nickname timestamp:time];
    }
    else if (msg.mediaType == kAVIMMessageMediaTypeLocation) {
        AVIMLocationMessage *locationMsg = (AVIMLocationMessage *)msg;
        xhMessage = [[XHMessage alloc] initWithLocalPositionPhoto:[UIImage imageNamed:@"Fav_Cell_Loc"] geolocations:locationMsg.text location:[[CLLocation alloc] initWithLatitude:locationMsg.latitude longitude:locationMsg.longitude] sender:userItem.nickname timestamp:time];
    }
        ...
        xhMessage = [[XHMessage alloc] initWithPhoto:image thumbnailUrl:nil originPhotoUrl:nil sender:userItem.nickname timestamp:time];
    }
        ...
        xhMessage = [[XHMessage alloc] initWithEmotionPath:path sender:userItem.nickname timestamp:time];
    }
        ...
        xhMessage = [[XHMessage alloc] initWithVideoConverPhoto:[XHMessageVideoConverPhotoFactory videoConverPhotoWithVideoPath:path] videoPath:path videoUrl:nil sender:userItem.nickname timestamp:time];
    } else {
        xhMessage = [[XHMessage alloc] initWithText:@"未知消息" sender:userItem.nickname timestamp:time];
        DLog("unkonwMessage");
    }
    
    xhMessage.avator = nil;
    xhMessage.avatorUrl = userItem.imageUrl;

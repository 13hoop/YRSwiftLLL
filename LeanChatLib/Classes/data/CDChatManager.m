...
- (void)openWithClientId:(NSString *)clientId ClientDic:(NSDictionary *)dic callback:(AVIMBooleanResultBlock)callback {
    _selfId = clientId;
    UserItem * userItem = [[UserItem alloc]init];
    NSLog(@"用户自己 存入数据库之前 dic = %@",[dic objectForKey:@"nickname"]);
    userItem.nickname = [dic objectForKey:@"nickname"];
    userItem.uuid = [dic objectForKey:@"uuid"];
    userItem.imageUrl = [dic objectForKey:@"avatar"];
    userItem.is_favorite = [dic objectForKey:@"is_favorite"];
...
- (void)fetchConvWithOtherId:(NSString *)otherId callback:(AVIMConversationResultBlock)callback {
    ...
    NSLog(@"clientId %@",[AVIMClient defaultClient].clientId);
...
//    id<CDUserModel> selfUser = [[CDChatManager manager].userDelegate getUserById:self.selfId];
    UserItem * item = [[FMDBManager sharedManager] getAllNicknameAndImageUrl:self.selfId];
    NSLog(@"self.selfId = %@",self.selfId);
    NSMutableDictionary *attributes = [NSMutableDictionary dictionary];
    // 云代码中获取到用户名，来设置推送消息, 老王:今晚约吗？
    if (self.selfId) {
        // 避免为空造成崩溃
        [attributes setObject:item.nickname forKey:@"username"];
    }
...
- (void)selectOrRefreshConversationsWithBlock:(AVIMArrayResultBlock)block {
    ...
    NSLog(@"CDChatManager selectOrRefreshConversationsWithBlock conversations = %@",conversations);
    if (refreshedFromServer == NO && self.connect) {
        ...
        for (AVIMConversation *conversation in conversations) {
         ...
         NSLog(@"CDChatManager selectOrRefreshConversationsWithBlock convids = %@",convids);
        [self fetchConvsWithConvids:convids callback:^(NSArray *objects, NSError *error) {
            NSLog(@"selectOrRefreshConversationsWithBlock objects = %@",objects);
            if (error) {
                block(conversations, nil);
            } else {
                refreshedFromServer = YES;
                [[CDConversationStore store] updateConversations:objects];
                 NSLog(@"[[CDConversationStore store] selectAllConversations] = %@",[[CDConversationStore store] selectAllConversations]);
...
- (void)findRecentConversationsWithBlock:(CDRecentConversationsCallback)block {
    [self selectOrRefreshConversationsWithBlock:^(NSArray *conversations, NSError *error) {
        NSLog(@"CDChatManager findRecentConversationsWithBlock conversations = %@",conversations);
